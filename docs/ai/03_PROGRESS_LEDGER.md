...
| CHK-R79 | Rich service todos — structured cards, communication threads, 5-state lifecycle, auto-reminders | in_progress | ⚠️ **MODULE UNDER CONSTRUCTION — IMPROVEMENT NEEDED.** Current state: grouped stop rendering works, re-seed button added, `seedOrderTodos` now fills placeholder todos for all 5 service types per stop even when no service items are configured. Known gaps: (a) todo cards are read-only placeholders — inline editing of supplier name, address, dates, contacts not yet implemented; (b) date picker fields not wired; (c) "person in charge" assignment not editable from dashboard; (d) stop service toggles (which services apply to which stop) not surfaced in UI. Review deferred by owner. Original implementation detail: (1) **`db/migrations/0052_booking_order_todos_service_fields.sql`**: 9 `ALTER TABLE` statements extending `booking_order_todos` with `service_type, service_item_id, status (pending/contacted/confirmed/cancelled/rebooked), person_in_charge, contact_name, contact_phone, contact_email, service_meta_json, last_reminded_at` + new index `idx_bot_status_order`. (2) **`db/migrations/0053_booking_todo_threads.sql`**: new `booking_todo_threads` table (`id, tenant_id, todo_id, order_id, channel, direction, message, sent_by, created_at`) with 2 indexes. (3) **`src/lib/bookingOps.js`** rewritten — `seedOrderTodos` now queries all 5 service item tables per stop (`stop_accommodations`, `stop_meals`, `stop_guides`, `stop_local_transports`, `stop_intercity_legs`) and seeds one rich todo per item with icon title, `service_type`, `service_item_id`, contact fields, and `service_meta_json` blob; falls back to one generic todo per stop if no service items exist. Added `runTodoReminders(env)` export: checks D-30/D-14/D-7/D-3 cadence + +30d from booking creation; groups per order; notifies via `notifyAgent`; updates `last_reminded_at` with 23h de-dupe window. (4) **`src/routes/bookings.js`** — `PATCH /order/:orderId/todos/:todoId` extended to accept `status` field (derives `done` from status); `POST /order/:orderId/todos` accepts service fields (`service_type`, `person_in_charge`, `contact_phone`, `contact_email`, `service_meta_json`); NEW `GET /order/:orderId/todos/:todoId/thread` returns thread entries; NEW `POST /order/:orderId/todos/:todoId/thread` adds thread entry + auto-advances todo status `pending → contacted` on first outbound contact. (5) **`src/index.js`** — imports `runTodoReminders` from `bookingOps.js`; adds `ctx.waitUntil(runTodoReminders(env))` to scheduled cron handler. (6) **`public/dashboard.html`** — complete todo card redesign: rich service cards with icon badge, contact buttons (📞 `tel:`, ✉️ `mailto:`, 💬 WhatsApp `wa.me`, 🔵 Zalo `zalo.me`, 📝 note), status selector with live chip update, collapsible thread panel with channel/direction selector + reply form; auto-bumps status chip when thread entry auto-advances todo; mobile-responsive (meta fields + subtext hidden at ≤640px). Deployed `a0a000d`. | 2026-04-xx | Applied migrations 0052+0053 locally + remote (2.11 MB DB). `stop_accommodations/stop_meals` use `contact_phone/contact_email`; `stop_guides/stop_local_transports/stop_intercity_legs` use `phone/email` — correctly mapped in `SERVICE_QUERIES` config. `done` column retained for backward compat; `status` is canonical. |
...
### Ops Board (dashboard.html) — CHK-R79 in_progress gaps

The structured service todo card rendering, status selector, contact buttons, and thread panel are all implemented. The following remain incomplete:

1. **Todo card inline edit** — `contact_name`, `contact_phone`, `contact_email`, `contact_address`, `person_in_charge` fields are rendered read-only in `.svc-meta-val` divs; no edit form is wired to `PATCH /order/:orderId/todos/:todoId`.
2. **Service date picker** — `service_meta_json.date` / `checkin_date` data exists in the DB but no `<input type="date">` is rendered in the card; editing a service date requires a direct API call.
3. **Person-in-charge assignment UI** — `person_in_charge` is shown as subtext-only beneath the card title; there is no dropdown or input field to assign or change it from the dashboard.
4. **Stop service type filter** — the ops board shows all todos per stop but offers no quick way to filter by service type (hotel-only, meals-only, guide, transport).
