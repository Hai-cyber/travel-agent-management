...
| CHK-R32 | Snippet Engine — syncAllSnippets.mjs | done | New script `scripts/syncAllSnippets.mjs`. Features: (1) Image path fix — relative src→absolute R2 URL; empty src→Unsplash by category. (2) 24 deep categories incl. hero-video, gallery-grid, booking-form, travel-itinerary, map-section. (3) Hybrid multi-label: snippet tagged with ALL matching categories. (4) Hero padding: `padding-top:80px` injected into hero/hero-video `<section>`. (5) Editable markers: `data-ve-text` on `<h1-h3>`,`<p>`; `data-ve-btn` on `<a>`. Up to 5/cat, diversity-first selection. Run: `node scripts/syncAllSnippets.mjs --r2-base https://pub-xxx.r2.dev --clear`. | 2026-03-30 | Supersedes seed-snippets-from-templates.mjs |
...
- CHK-R32: `scripts/syncAllSnippets.mjs` — full-featured extractor for all 20 Cruip templates. 24 categories, hybrid multi-label, R2 image paths, Unsplash fallback, hero padding, editable markers.
