...
- CHK-R25/R26 storefront follow-up completed: `Check availability` on tour detail pages now opens a dedicated public Booking View that mirrors the conversational calculator logic from `public/tour-config.html`, uses canonical tour pricing APIs, opens as a themed sidebar on desktop, and routes to the booking page inline view on mobile.
- Payment handoff hook captured: `public/tour-booking-view.js` now emits `travelagent:public-booking-quote-ready` and `travelagent:public-booking-payment-intent`, and the reserved payment CTA keeps `data-payment-*` payload fields for future checkout integration.
- Storefront payment-flow follow-up completed: the public Booking View now opens a payment sheet, captures guest identity, reads tenant payment settings, creates real booking orders for payment-compliant tenants via `POST /api/bookings/order`, and falls back to a clearly labeled `demo payment` success path when no electronic gateway is configured yet.
- Pricing display stabilization completed: added `docs/ai/27_PRICING_DISPLAY_POLICY.md`, audited storefront money render paths, normalized universal storefront price rendering through a shared formatter, and extended synced pricing snapshots/cards with explicit currency metadata so display can stay canonical without enabling FX conversion.
