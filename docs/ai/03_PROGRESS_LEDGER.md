...
| CHK-R53 | Booking lifecycle email notifications | done | Built `src/lib/bookingEmails.js` with 3 dispatch functions (`dispatchBookingCreatedEmail`, `dispatchProofUploadedEmail`, `dispatchBookingConfirmedEmail`). Hooked into `bookings.js` via `executionCtx.waitUntil` (non-blocking) at: order creation, agent-side proof upload, guest portal proof upload, and confirm-receipt. GAS updated to `gas-booking-mailer-v1` (Version 6, 2026-04-13) — handles all event types generically. Root cause of "Unauthorized": GAS reads POST body as Latin-1 for HMAC verify while Workers sign with UTF-8 — fixed by escaping all `\u0080–\uFFFF` to `\uXXXX` in payload before signing. Commits: `b7f3a6f` (scaffold) + `0a4576e` (fix). All 3 test emails confirmed received at `saophuongbac@gmail.com`. | 2026-04-13 | Fix applies universally: Vietnamese names, em-dashes, and all non-ASCII in subject/body are safe to use. |
