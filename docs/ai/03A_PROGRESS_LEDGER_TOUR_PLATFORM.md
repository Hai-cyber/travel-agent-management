...
| CHK-P01 | PaymentAdapter interface + provider routing | not_started | P2 — payments | Abstract adapter layer: `createPaymentLink(order) → {url}`, `handleWebhook(raw,sig) → {order_id,status}`. Tenant config resolves which adapters are active via `payment_config_json.enabled_methods`. |
| CHK-P02 | Stripe adapter — card payments + webhook confirm | not_started | P2 — payments | `POST /api/pay/stripe/create` → Stripe Checkout session. `POST /api/pay/stripe/webhook` → verify sig + confirm booking order. Env vars: `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`. |
| CHK-P03 | Xendit adapter — SEA e-wallets + VA + webhook | not_started | P2 — payments | `POST /api/pay/xendit/create` → Xendit invoice. `POST /api/pay/xendit/webhook` → verify token + confirm booking order. Env var: `XENDIT_API_KEY`, `XENDIT_WEBHOOK_TOKEN`. Channels: DANA, OVO, GCASH, MAYA, FPX, QRIS, VA_BCA, VA_BNI. |
| CHK-P04 | Guest payment checkout flow | not_started | P2 — payments | `public/pay.html` — method selector rendered from tenant `enabled_methods`. Card → Stripe Checkout redirect. E-wallet → Xendit invoice redirect. Manual stays as always-available fallback. `booking_orders.payment_provider` + `payment_session_id` columns (migration 0081). |