...
| CHK-R123 | Tour gift cards — tenant-issued QR cards + price application | done | migration 0094 adds `tour_gift_cards` plus applied-order columns; `src/routes/pricing.js` adds tenant gift-card CRUD + QR SVG generation and optional `gift_card_id_code` application on quote/calculate; `src/routes/bookings.js` now snapshots and consumes gift-card balance on booking create; `public/tour-config.html` adds gift-card generation and calculator apply UI | 2026-06-01 | local `npm test` covers quote-time application and booking-time balance consumption |
| CHK-R124 | Tenant discount coupons — marketing/referral codes across quote, draft, and order | done | migration 0095 adds `tenant_discount_coupons` plus booking-order coupon columns; `src/routes/pricing.js` adds `GET/POST/PATCH /api/pricing/discount-coupons` and optional `discount_coupon_code` application on quote/calculate; `src/routes/bookings.js` snapshots coupon data on drafts and persists applied coupon metadata plus usage increments on order create; `public/tour-config.html`, `public/booking-widget.js`, and `public/tour-booking-view.js` add tenant management and guest coupon entry; `src/index.js` now keeps `/api/pricing/calculate` public for booking widgets | 2026-06-01 | local + remote D1 migration applied; production deploy live-verified create/list/calculate/draft/order coupon flow with persisted booking-order coupon metadata and `uses_count` increment; temporary demo smoke draft/order/coupon rows were cleaned from remote after verification |
...
| CHK-R111 | Reporting dashboard — revenue + bookings + per-tour stats | done | `src/routes/reports.js` + `public/reports.html`; GET /api/reports/overview?period=30d|90d|12m|all; 7 KPI cards, bar chart, top tours table, status breakdown, recent 10 orders | 2026-04-20 | Deployed commit f6be0d3 |
...
| CHK-R113 | Product tier gating — dashboard UI + API guards | in_progress | shared platform packaging | Baseline UI/runtime tier gating is now live: `/api/billing/status` exposes `product_tier_key` + `available_membership_targets`; dashboard billing pane is tier-aware; soft locks now route tenants into the billing pane across dashboard, team, ops, suppliers, inbox, and properties-engine. Remaining work is the deeper API-guard layer for feature-specific writes such as payment-config and booking-form level rejection where packaging must be enforced server-side in addition to UX routing. |
| CHK-R114 | Distribution helper — Listing Package Export API + distribution.html | done | `GET /api/distribution/tours/:id/listing-package` assembles OTA card, social snippet, travel-agent brief. `public/distribution.html` with tab strip + copy buttons. Dashboard sidebar link added (`data-tier="tours"`). | 2026-04-20 | src/routes/distribution.js deployed |
| CHK-R115 | Distribution helper — Channel Readiness Checklist | done | `GET /api/distribution/tours/:id/channel-readiness` scores tour for 3 channels (own site, OTA, social). Readiness cards with ✅/○ per check. Rendered in distribution.html. | 2026-04-20 | part of distribution.js |
| CHK-R116 | Distribution helper — Booking Source Tracking | done | Migration `0080_booking_source.sql` adds `booking_source TEXT` to `booking_orders`. Applied to remote D1. | 2026-04-20 | column available; inbox.html confirm UI update deferred |
| CHK-R117 | Distribution helper — Social media automation | not_started | DEFERRED — under consideration | Auto-post or schedule social content (tour card + caption) to Facebook/Instagram. Depends on Meta Business API access. Defer until business accounts are ready. |
| CHK-R118 | Platform-wide tour search — /api/universal/search | done | Existing route in universalSites.js fixed: now accepts `on_sale` status (was `published`-only) and removed `public_indexing_enabled = 1` gate. JOINs `tenant_universal_pages` for correct page slug in search result URLs. | 2026-04-20 | deployed |
| CHK-R119 | Subdomain /t/:slug routing fix | done | Subdomain handler in index.js now strips `/t/` prefix before slugify, so `/t/my-tour-slug` correctly resolves to the `tenant_universal_pages` row. | 2026-04-20 | deployed |
| CHK-P01 | PaymentAdapter interface + provider routing | done | P2 — payments | `src/lib/paymentAdapters.js`: `createStripeCheckout`, `createXenditInvoice`, `verifyStripeWebhook`, `verifyXenditWebhook`. Resolves which adapter to use based on `provider` param. |
| CHK-P02 | Stripe adapter — card payments + webhook confirm | done | P2 — payments | `POST /api/pay/create?provider=stripe` → Stripe Checkout session (via `createStripeCheckout`). `POST /api/pay/stripe/webhook` → verify `Stripe-Signature` + confirm booking. Env vars: `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`. |
| CHK-P03 | Xendit adapter — SEA e-wallets + VA + webhook | done | P2 — payments | `POST /api/pay/create?provider=xendit` → Xendit Invoice v2 (via `createXenditInvoice`). `POST /api/pay/xendit/webhook` → verify `x-callback-token` + confirm booking. Env vars: `XENDIT_API_KEY`, `XENDIT_WEBHOOK_TOKEN`. |
| CHK-P04 | Guest payment checkout flow | done | P2 — payments | `public/pay.html` — loads order via `/api/bookings/public/:token`, renders enabled methods, redirects to Stripe/Xendit. `pay-success.html` + `pay-cancel.html`. `GET /api/pay/status`. Migration `0081_payment_sessions.sql` adds `payment_provider`, `payment_session_id`, `payment_link_url`, `payment_link_expires_at` to `booking_orders`. Applied to remote D1. |
| CHK-R134 | Membership settlement core — provider-agnostic billing intent and manual approval baseline | done | shared platform billing | Runtime now includes `membership_billing_intents`, tenant routes for create/current/mark-submitted, admin review routes, `app_settings`/env-backed manual settlement config resolution, and activation of tenant entitlement from a settled intent. Production D1 migration `0104_membership_billing_intents.sql` is now applied remotely on `travel_agent_db`, so the live platform uses persisted billing intents rather than the temporary missing-table rescue path. Sample apply command: `npx wrangler d1 migrations apply travel_agent_db --remote --config ./wrangler.jsonc`. |
| CHK-R135 | `tours_pro` membership completion | done | shared platform billing | Dashboard billing pane now exposes manual settlement for `tour_operator_pro`, trial/upgrade CTA routes into the billing pane instead of assuming Stripe first, and local smoke verified tenant activation into the paid tour tier without Stripe being mandatory. Runtime follow-up also compacted the upgrade entry into a top-bar modal and made `Next` auto-create/reuse the selected membership intent so the billing pane lands directly on actionable instructions. |
| CHK-R136 | `hotel_pro` membership completion | in_progress | shared platform billing + hotel packaging | Membership runtime now allows `starter_landing -> hotel_operator_pro` intent creation through the shared settlement core, the billing pane can present Hotel Pro as a self-serve target, and the property engine now routes lower tiers back into the billing pane. Production now has the required `membership_billing_intents` table, so remaining work is hotel-package polish and dedicated live validation rather than schema rescue. |
| CHK-R137 | `all_in_one` membership completion | in_progress | shared platform billing + combined packaging | Membership runtime now allows `tour_operator_pro -> tour_hotel_suite` and `hotel_operator_pro -> tour_hotel_suite` through the shared settlement core, the billing pane can present Suite upgrade options, and cross-surface soft locks now steer both tour and hotel tenants toward suite when they touch the other engine. Production D1 is ready for persisted suite intents; remaining work is suite-specific UX polish and final live validation. |