...
| CHK-R113 | Product tier gating — dashboard UI + API guards | not_started | DEFERRED — implement after hotel engine complete | See `38_PLATFORM_PACKAGING_AND_TIERS.md` § Concrete tier map for full spec. No DB migration needed (`tenants.product_tier_key` already exists). Scope: expose key in `/api/billing/status`; dashboard JS hides `[data-tier=hotel]` on `tours_pro` and vice versa; payment config + booking form API guards reject `starter_landing`. Convention: add `data-tier="hotel"` or `data-tier="tours"` to sidebar `<li>` items as hotel pages are built. |
