...
| CHK-R58 | Trial countdown banner + Upgrade button restored + i18n localized | done | Historical banner fix. Note: the original Stripe-first `#btn-upgrade -> /api/billing/checkout` behavior has since been superseded by the provider-agnostic membership billing pane. The root-cause analysis and i18n work remain historically accurate. | 2026-04-14 | Archive only — do not treat this entry as the current upgrade path. |
...
| CHK-R74 | Refund policy expansion + payment trust badges | done | Historical static-page work. The original "Secured by Stripe" membership-billing trust badges have since been replaced by provider-agnostic billing wording on current public pages. | 2026-04-15 | Archive only — guest payment/domain-purchase Stripe references elsewhere may still be current. |
...
| CHK-R76 | Post-payment app — full billing lifecycle | done | Historical Stripe lifecycle work. Stripe portal/webhook flows still exist as a legacy/optional billing path, but tenant membership activation is no longer defined solely by this Stripe-only flow. The old dashboard "Upgrade -> checkout" assumption is superseded by the provider-agnostic membership billing pane. | 2026-04-16 | Archive only — keep this as historical Stripe lifecycle context, not as the current membership architecture. |
