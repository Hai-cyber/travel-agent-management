...
- no undo path for `checked_out`, no admin/manual confirm route yet, and no richer post-stay settlement flow tied to folios yet
...
- expand pricing v2 from seasons + aggregate occupancy surcharges into richer rate plans, per-room guest assignment pricing, and package/rule layers only after the current season model stays stable
- add undo/operational recovery for `checked_out` and richer no-show / reinstatement audit trails
