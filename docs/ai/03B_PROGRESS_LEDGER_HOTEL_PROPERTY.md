...
| CHK-R110 | Housekeeping V1 stayover refresh automation | in_progress | Added migration `0090_housekeeping_task_kind_service_date.sql`, splitting `housekeeping_tasks` into `departure_clean` vs `stayover_refresh` with `service_date` for per-room/day idempotence. `src/routes/properties.js` now generates local-09:00 stayover refresh tasks for checked-in, assigned, non-departing rooms, keeps those pseudo states out of `room_state_events`, and returns actor-aware housekeeping payloads with `task_kind`, `actor_role`, `room_states`, and effective room state. `public/property-staff.html` now renders `Refresh Needed` / `Refreshing` / `Refreshed` separately from turnover dirty/inspection flow. `src/index.js` now runs scheduled housekeeping automation on the existing 15-minute cron tick. | 2026-04-22 | Repo build is in place, but remote migration + deploy are still pending. Narrow validation passed for diagnostics and direct import of `src/routes/properties.js`. Full `npm test` remains blocked by an older local migration issue in `0052_booking_order_todos_service_fields.sql` (`duplicate column name: service_type`). |
