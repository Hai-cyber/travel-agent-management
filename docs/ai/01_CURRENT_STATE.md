...
- `GET /api/properties/:propertyId/housekeeping-tasks` — authenticated housekeeping board read; returns active housekeeping task rows with room, guest, priority, task status, and effective room-state projection
- `POST /api/properties/:propertyId/housekeeping-tasks/sync` — authenticated housekeeping sync baseline; backfills departure-clean tasks for checked-out stays on the selected `board_date`
- `PATCH /api/properties/:propertyId/housekeeping-tasks/:taskId` — authenticated housekeeping task patch; updates task status/assignee/note and records corresponding `room_state_events`
...
- `public/property-staff.html` — staff-facing property operations surface; separates front desk, housekeeping baseline, maintenance desk, guest support, and POS/folio boundary lanes from tenant configuration. Uses the property reservation board feed plus current reservation detail/status actions, room-unit operational status controls, and a read-only view of the tenant-configured addon preset catalog in the `POS / Folio` lane, including the onsite-only / airport-pickup exception policy.
...
- `public/property-staff.html` now also reads live housekeeping tasks into the staff desk. Rack cells can show `dirty`, `cleaning`, and `inspected` from housekeeping task state, the Housekeeping tab can `Sync` and advance tasks through `Start`, `Inspect`, and `Ready`, and night audit now posts room-rate charges into folios through runtime rather than leaving room charges manual-only.
