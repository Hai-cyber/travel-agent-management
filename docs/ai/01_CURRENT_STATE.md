...
- `public/properties-engine.html` — property builder/admin surface now also includes pricing v2 controls for rate seasons, seasonal room prices, multi-room occupancy-aware quote output with optional per-room guest assignments, and authenticated reservation lifecycle controls for hold release, reservation lookup, rebook, no-show, undo-status, check-in/out, and early check-out. The page now uses parent tabs (`Availability`, `Ops`) plus room-type tabs to reduce workspace sprawl on dense room-type operations.
