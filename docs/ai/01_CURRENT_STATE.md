...
- The guest-sheet reservation form now consumes that planner directly. Reservation mode exposes a `Rooms` field and a planner preview panel so staff can see candidate plans, preferred-lane conflicts, blocking overlaps, allotment-consumption summaries, and alternate recommendations before creating the reservation. Walk-in mode stays single-room only and bypasses the bulk-planning semantics.
