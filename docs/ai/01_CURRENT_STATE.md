...
- `GET /api/properties/:propertyId/addon-service-presets` — tenant-scoped property addon catalog read; returns reusable addon service presets such as transfers, breakfast, laundry, late checkout, and extra bed presets for one property
...
- `public/properties-engine.html` — tenant-facing property configuration surface; includes property content framing, inventory structure, pricing setup, config-side operational tools, and a new addon service preset catalog in the `Pricing` workspace. The page now uses parent tabs (`Content`, `Inventory`, `Pricing`, `Ops`) plus room-type tabs to reduce workspace sprawl on dense tenant configuration work.
- `public/property-staff.html` — staff-facing property operations surface; separates front desk, housekeeping baseline, maintenance desk, guest support, and POS/folio boundary lanes from tenant configuration. Uses the property reservation board feed plus current reservation detail/status actions, room-unit operational status controls, and a read-only view of the tenant-configured addon preset catalog in the `POS / Folio` lane.
