...
### Product intent
A tour-operations MVP centered on tour_stops (canonical itinerary segments), computed schedules, 5 service groups (service items linked to tour_stop_id), communication threads, tasks/reminders, pricing (tenant_seasons, pricing_segments, pax_bands, tour_prices), visual config, and itinerary composition.
