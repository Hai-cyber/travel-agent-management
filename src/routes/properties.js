...
export async function handleCheckPropertyAvailability(request, env, params) {
  ...
  try {
    const availability = await availabilityCalculateAvailability(
      env,
      tenantId,
    ...
    return jsonResponse(availabilityBuildAvailabilityPayload(availability));
  } catch (error) {
...
export async function handlePlanPropertyReservation(request, env, params) {
  ...
  try {
    ...
    const conflicts = await availabilityBuildReservationPlanningConflicts(
      env,
      tenantId,
      parsedRequest.propertyId,
      parsedRequest.roomTypeId,
    ...
    const preferredRoom = parsedRequest.preferredRoomUnitId
        ? await availabilityBuildPreferredRoomUnitConflicts(
          env,
          tenantId,
          parsedRequest.propertyId,
          parsedRequest.roomTypeId,
    ...
    const payload = availabilityBuildReservationPlanningPayload(availability, conflicts, {
      preferredRoomUnitId: parsedRequest.preferredRoomUnitId,
      preferredRoomUnit: preferredRoom.roomUnit,
      preferredRoomConflicts: preferredRoom.conflicts,
      allotmentConsumption: availabilityBuildAllotmentConsumptionSummary(activeAllotment, parsedRequest.roomsRequested),
    });
...
export async function handlePlanPropertyReservationExtension(request, env, params) {
  ...
  try {
    ...
    return jsonResponse({
      ...
      planner: availabilityBuildReservationPlanningPayload(availability, conflicts),
    });
  } catch (error) {
  ...
  try {
    ...
    if (availability.shortageDates.length) {
      return jsonResponse({
        ...
        availability: availabilityBuildAvailabilityPayload(availability).availability,
      }, 409);
    }
    ...
    const refreshedAvailability = await availabilityCalculateAvailability(
      env,
      tenantId,
      parsedRequest.propertyId,
      parsedRequest.roomTypeId,
    ...
    return jsonResponse({
      ...
      availability: refreshedAvailability.error ? null : availabilityBuildAvailabilityPayload(refreshedAvailability).availability,
    }, 201);
  } catch (error) {
...
export async function handleCreatePropertyReservation(request, env, params) {
  ...
  try {
    ...
    const selectedPlan = availabilitySelectBestPlan(availability);
    ...
    const allotmentConsumption = availabilityBuildAllotmentConsumptionSummary(activeAllotment, parsedRequest.roomsRequested);
