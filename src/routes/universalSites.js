...
function normalizeSite(siteRow) {
  return siteRow ? {
    ...
    target_currency: siteRow.target_currency || siteRow.base_currency || 'USD',
    base_currency: siteRow.base_currency || 'USD',
    exchange_rate: siteRow.exchange_rate,
...
function buildTourRuntimeCollections(tenantId, tours, syncedRows, hotels = [], priceLookup = new Map(), currencyInfo = {}) {
...
async function requireTenant(c) {
  ...
  const tenant = await c.env.DB
    .prepare('SELECT id, name, template_id, site_config, default_locale, base_currency, target_currency, exchange_rate FROM tenants WHERE id = ?')
    .bind(tenantId)
    .first();
...
async function getSiteBundle(tenantId, tenant, db, env = null) {
  ...
  site.default_lang = tenant.default_locale || site.default_lang || 'en';
  site.base_currency = tenant.base_currency || site.base_currency || 'USD';
  site.target_currency = tenant.target_currency || site.target_currency || site.base_currency || 'USD';
  site.exchange_rate = Number(tenant.exchange_rate || site.exchange_rate || 1);
